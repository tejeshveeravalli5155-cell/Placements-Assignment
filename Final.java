...
public class Final {
    ...
    static void solve(String path) throws Exception {
        ...
        Set<BigInteger> good = witnesses.get(best);
        Set<BigInteger> bad = new TreeSet<>();
        for (BigInteger[] p : points) if (!good.contains(p[0])) bad.add(p[0]);

        System.out.println("=== " + path + " ===");
        System.out.println("n=" + n + ", k=" + k);
        System.out.println("constant term c = " + best);
        System.out.println("consistent (good) point x-values: " + good);
        System.out.println("corrupted point x-values: " + (bad.isEmpty() ? "none" : bad));
        System.out.println();
    }
